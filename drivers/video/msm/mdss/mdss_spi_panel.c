...
int update_epd(struct mdss_panel_data *pdata,int flash_time)
{
	...
	kobject_uevent_env(&g_dev->kobj, KOBJ_CHANGE, envp2); // send uevent busy=1
...
0x00, 0x00, 0x00, 0x00, 0x00};
	   //LUTD
			char LUTD[16386];
			char parm16[16386]={0x21,0x2B, 0x20, 0x20, 0x00, 0x20, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static struct attribute *epd_attrs[] = {
	...
	&dev_attr_battery_full.attr,
	NULL
};
//[4101][Raymond]picture of the battery full for off-charging
...
static int mdss_spi_panel_probe(struct platform_device *pdev)
{
	...
	if (!ctrl_name)
		pr_debug("%s:%d, Ctrl name not specified\n",
			__func__, __LINE__);
	else
